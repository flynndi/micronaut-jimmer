...
import io.micronaut.jimmer.repository.parser.Query;
...
public class QueryParserTest {
    ...
    public void testSimplePredicate() {
        assertQuery(
                Query.of(new Context(), new Source("findByName"), ImmutableType.get(Book.class)),
                "Query{"
                        + "--->action=FIND, "
                        + "--->limit=2147483647, "
                        + "--->distinct=false, "
    ...
    public void testComplexPredicates() {
        assertQuery(
                Query.of(
                        new Context(),
                        new Source(
                                "findByNameAndPriceBetweenOrStoreNameStartsWithAndStoreNameIgnoreCase"),
                        ImmutableType.get(Book.class)),
                ""
                        + "Query{"
                        + "--->action=FIND, "
    ...
    public void testComplexQuery() {
        assertQuery(
                Query.of(
                        new Context(),
                        new Source(
                                "findTop10ByNameContainsAndStoreNameStartsWithOrderByNameDescStoreNameAsc"),
                        ImmutableType.get(Book.class)),
                "Query{"
                        + "--->action=FIND, "
                        + "--->limit=10, "
    ...
    public void testFindImplicitTop() {
        assertQuery(
                Query.of(
                        new Context(),
                        new Source(
                                "findTopByNameContainsAndStoreNameStartsWithOrderByNameDescStoreNameAsc"),
                        ImmutableType.get(Book.class)),
                "Query{"
                        + "--->action=FIND, "
                        + "--->limit=1, "
        ...
        IllegalArgumentException ex =
                Assertions.assertThrows(
                        ...
                        () -> {
                            Query.of(
                                    new Context(),
                                    new Source("deleteXByName"),
                                    ImmutableType.get(Book.class));
                        });
        Assertions.assertEquals("Illegal method name \"delete[X]ByName\"", ex.getMessage());
    }
        ...
        IllegalArgumentException ex =
                Assertions.assertThrows(
                        ...
                        () -> {
                            Query.of(
                                    new Context(),
                                    new Source("deleteByNameOrderByName"),
                                    ImmutableType.get(Book.class));
                        });
        Assertions.assertEquals(
                "Illegal method name \"deleteByName[OrderByName]\"", ex.getMessage());
    }
        ...
        IllegalArgumentException ex =
                Assertions.assertThrows(
                        ...
                        () -> {
                            Query.of(
                                    new Context(),
                                    new Source("findByAndName"),
                                    ImmutableType.get(Book.class));
                        });
        Assertions.assertEquals(
                "Cannot resolve the property name \"findBy[AndName]\" "
                        + "by \"io.micronaut.jimmer.java.it.entity.Book\"",
                ex.getMessage());
    }
