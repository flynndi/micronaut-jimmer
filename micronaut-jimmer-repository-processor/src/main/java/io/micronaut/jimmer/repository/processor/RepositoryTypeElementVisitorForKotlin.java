...
import io.micronaut.jimmer.repository.support.KRepositoryImpl;
...
final class RepositoryTypeElementVisitorForKotlin {

    RepositoryTypeElementVisitorForKotlin() {}
    ...
    public void visitClass(ClassElement element, VisitorContext context) {
        ...
        ClassTypeDef jRepositoryImplClassTypeDef = ClassTypeDef.of(KRepositoryImpl.class);
        ...
        ParameterDef parameterDef =
                ParameterDef.builder("var1", TypeDef.of(KSqlClient.class))
                        .addAnnotation(
        ...
        for (MethodElement method : methods) {
            ...
            FieldDef fieldDef =
                    FieldDef.builder(generatorMethodName)
                            ...
                            .addModifiers(List.of(Modifier.PRIVATE, Modifier.STATIC))
                            .build();
            ...
            methodCallArgs.add(ExpressionDef.constant(method.getName())); // 方法名字符串
                ...
                Arrays.stream(method.getParameters())
                        .forEach(
                                arg -> {
                                    ...
                                    // Handle parameterized types - extract raw type
                                    if (typeDef
                                            instanceof ClassTypeDef.Parameterized parameterized) {
                                        // For parameterized types, use the raw type
                                        ClassTypeDef rawType = parameterized.rawType();
                                        // Always use non-nullable version for class literals
                                        parametersTypes.add(
                                                ExpressionDef.constant(
                                                        getNonNullableVersion(rawType)));
                                    } else if (typeDef instanceof TypeDef.Primitive primitive) {
                                        // For primitive types, generate correct Kotlin class
                                        // references
                                        ClassTypeDef wrapperType =
                                                getKotlinPrimitiveWrapper(primitive);
                                        parametersTypes.add(ExpressionDef.constant(wrapperType));
                                    } else if (typeDef instanceof ClassTypeDef classType) {
                                        // Always use non-nullable version for class literals
                                        parametersTypes.add(
                                                ExpressionDef.constant(
                                                        getNonNullableVersion(classType)));
                                    } else {
            ...
            MethodDef newMethod =
                    MethodDef.override(method)
                            .build(
                                    (aThis, methodParameters) -> {
                                        ExpressionDef pageableArg = ExpressionDef.nullValue();
                                        ExpressionDef sortArg = ExpressionDef.nullValue();
                                        ExpressionDef specificationArg = ExpressionDef.nullValue();
                                        ExpressionDef fetcherArg = ExpressionDef.nullValue();
                                        ExpressionDef classTypeArg = ExpressionDef.nullValue();
                                        ...
                                        if (!(pageableArg instanceof ExpressionDef.Constant constant
                                                && constant.value() == null)) {
                                            pageableArg = pageableArg.cast(paginationType);
                                        }
                                        if (!(sortArg instanceof ExpressionDef.Constant constant
                                                && constant.value() == null)) {
                                            sortArg = sortArg.cast(sortType);
                                        }
                                        if (!(specificationArg
                                                        instanceof ExpressionDef.Constant constant
                                                && constant.value() == null)) {
                                            specificationArg =
                                                    specificationArg.cast(specificationType);
                                        }
                                        if (!(fetcherArg instanceof ExpressionDef.Constant constant
                                                && constant.value() == null)) {
                                            fetcherArg = fetcherArg.cast(fetcherType);
                                        }
                                        if (!(classTypeArg
                                                        instanceof ExpressionDef.Constant constant
                                                && constant.value() == null)) {
                                            classTypeArg = classTypeArg.cast(classType);
                                        }
                                        ...
                                        ExpressionDef newObjectArray =
                                                TypeDef.array(TypeDef.OBJECT.makeNullable())
                                                        .instantiate(filteredParameters);
                                        List<ExpressionDef> executeArgs = new ArrayList<>();

                                        VariableDef.Field sqlField =
                                                new VariableDef.Field(
                                                        aThis.superRef(),
                                                        "sql",
                                                        sqlClientImplementorType);
                                        VariableDef.Field javaClientField =
                                                new VariableDef.Field(
                                                        sqlField,
                                                        "javaClient",
                                                        sqlClientImplementorType);
                                        executeArgs.add(javaClientField);
    ...
    private ClassTypeDef getNonNullableVersion(ClassTypeDef classType) {
        if (!classType.isNullable()) {
            return classType;
        }

        // Create non-nullable version based on the specific implementation
        if (classType instanceof ClassTypeDef.JavaClass javaClass) {
            return new ClassTypeDef.JavaClass(javaClass.type(), false);
        } else if (classType instanceof ClassTypeDef.ClassName className) {
            return new ClassTypeDef.ClassName(className.name(), className.isInner(), false);
        } else if (classType instanceof ClassTypeDef.ClassElementType elementType) {
            return new ClassTypeDef.ClassElementType(elementType.classElement(), false);
        }
    ...
    private ClassTypeDef getKotlinPrimitiveWrapper(TypeDef.Primitive primitive) {
        return switch (primitive.name()) {
            case "int" -> ClassTypeDef.of("kotlin.Int");
            case "long" -> ClassTypeDef.of("kotlin.Long");
            case "boolean" -> ClassTypeDef.of("kotlin.Boolean");
            case "byte" -> ClassTypeDef.of("kotlin.Byte");
            case "short" -> ClassTypeDef.of("kotlin.Short");
            case "char" -> ClassTypeDef.of("kotlin.Char");
            case "float" -> ClassTypeDef.of("kotlin.Float");
            case "double" -> ClassTypeDef.of("kotlin.Double");
            default ->
                    throw new IllegalStateException(
                            "Unsupported primitive type: " + primitive.name());
        };
    }
}
