...
class QueryMethodParser {
    ...
    private QueryMethod parse0() {
        ...
        if (query.getAction() == Query.Action.FIND) {
            ...
            if (query.getSelectedPath() != null) {
                ...
                ReturnedElementType returnedElementType =
                        returnedElementType(actualElementType, entityType);
        ...
        if (specificationIndex != -1) {
            if (query.getAction() != Query.Action.FIND
                    && query.getAction() != Query.Action.COUNT
                    && query.getAction() != Query.Action.EXISTS) {
                throw new IllegalArgumentException(
                        "The method must be query method when there is a specification parameter");
            }
            ...
            if (!(specificationType instanceof ParameterizedType)) {
                ...
                throw new IllegalArgumentException(
                        "The specification parameter must be parameterized type");
            }
        ...
        if (fetcherParamIndex != -1) {
            ...
            if (!(fetcherType instanceof ParameterizedType)) {
                throw new IllegalArgumentException(
                        "The fetcher parameter must be parameterized type");
            }
        ...
        if (viewTypeParamIndex != -1 && query.getAction() != Query.Action.FIND) {
            throw new IllegalArgumentException(
                    "The method must be query method when there is a view type parameter");
        }
        if (pageableParamIndex != -1 && query.getAction() != Query.Action.FIND) {
            throw new IllegalArgumentException(
                    "The method must be query method when there is a pageable parameter");
        }
        if (sortParamIndex != -1 && query.getAction() != Query.Action.FIND) {
            throw new IllegalArgumentException(
                    "The method must be query method when there is a sort parameter");
        }
    ...
    private ReturnedElementType returnedElementType(Type type, Class<?> entityType) {
        if (type instanceof Class<?>) {
            ...
            if (clazz == entityType) {
                ...
                Type[] typeArguments =
                        GenericTypeUtils.resolveSuperTypeGenericArguments(clazz, View.class);
    ...
    private Predicate resolve(PropPredicate propPredicate) {
        ...
        switch (propPredicate.getOp()) {
            ...
            case BETWEEN:
            case NOT_BETWEEN:
                return ((PropPredicate.Unresolved) propPredicate)
                        .resolve(nextParam(propPredicate), nextParam(propPredicate));
            default:
                throw new AssertionError(
                        "Internal bug: unexpected predicate op: " + propPredicate.getOp());
        }
    }
    ...
    private Param nextParam(PropPredicate predicate) {
        paramIndex++;
        while (paramIndex < parameterTypes.length
                && isImplicitParameterType(parameterTypes[paramIndex])) {
            paramIndex++;
        }
        ...
        if (predicate.getOp() == PropPredicate.Op.IN
                || predicate.getOp() == PropPredicate.Op.NOT_IN) {
            boolean valid = false;
            ...
            if (parameterizedType != null) {
                if (parameterizedType.getRawType() == Collection.class
                    ...
                    valid =
                            Classes.boxTypeOf(predicate.getPath().getType())
            ...
            if (!valid) {
                expectedTypeName =
                        "Collection<"
                                + Classes.boxTypeOf(predicate.getPath().getType()).getName()
                                + '>';
                actualTypeName =
