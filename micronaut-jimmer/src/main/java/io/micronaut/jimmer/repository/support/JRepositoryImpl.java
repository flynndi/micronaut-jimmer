...
public class JRepositoryImpl<E, ID> implements JRepository<E, ID> {
        ...
        if (entityType != null) {
            ...
            Class<?>[] typeArguments =
                    GenericTypeUtils.resolveSuperTypeGenericArguments(
                            this.getClass(), JRepository.class);
    ...
    public Page<E> findAll(int pageIndex, int pageSize) {
        return this.<E>createQuery(null, null, null, null)
                .fetchPage(pageIndex, pageSize, MicronautPageFactory.getInstance());
    }
    ...
    public int deleteAll(@NotNull Iterable<? extends E> entities, DeleteMode mode) {
        return sqlClient
                ...
                .deleteAll(
                        entityType,
                        Utils.toCollection(entities).stream()
                                .map(
                                        it ->
                                                ImmutableObjects.get(
                                                        it, immutableType.getIdProp().getId()))
    ...
    public void deleteAll() {
        Mutations.createDelete(sqlClient, immutableType, (d, t) -> {}).execute();
    }
    ...
    private <X> ConfigurableRootQuery<?, X> createQuery(
        ...
        MutableRootQueryImpl<Table<?>> query =
                new MutableRootQueryImpl<>(
                        sqlClient, immutableType, ExecutionPurpose.QUERY, FilterLevel.DEFAULT);
        ...
        if (sort != null) {
            query.orderBy(MicronautOrders.toOrders(table, sort));
        }
        return query.select(
                fetcher != null
